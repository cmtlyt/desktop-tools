...
`;var E=(e=>(e.START="start",e.PAUSE="pause",e.RELOAD="reload",e))(E||{});const ee=new me;function Y(e){ee.next(e),v.event("ELSFK-action",e)}const Ue=fe(ee);function Ge(){const{gameStatus:e}=F("gameStatus"),[t,r]=m.useState(!1),n=e===h.RUNNING;return a.jsxs(a.Fragment,{children:[a.jsx(we,{buttons:[{text:n?"暂停":"开始",onClick(){Y({id:`elsfk-${n?"pause":"start"}`,type:n?E.PAUSE:E.START})}},{text:"重置",onClick(){Y({id:"elsfk-reload",type:E.RELOAD})}},{text:"游戏记录",onClick:()=>r(!0)}]}),a.jsx(pe,{open:t,storeKey:J,onClose:()=>r(!1),children:s=>a.jsx(de,{$direction:M.COLUMN,$gap:"0.5",children:a.jsxs(he,{$alignItems:O.CENTER,$justifyContent:$.BETWEEN,children:[a.jsxs("span",{children:["分数: ",s.score]}),a.jsx(Se,{format:"yyyy-MM-DD hh:mm:ss",children:s.time})]})},s.gameId)})]})}function mt(){const{row:e,col:t,moveAddRow:r}=F(["row","col","moveAddRow"]),n=e+r,s=m.useRef(500),l=m.useRef(!1),c=m.useMemo(()=>Ce(n,e),[n,e]),o=m.useRef(c(!1)),i=m.useRef(c(!0)),p=m.useRef({}),R=m.useRef({}),[U,te]=m.useState(b(r,o.current,i.current)),[re,G]=m.useState([]),S=m.useRef(0),H=m.useMemo(()=>(1<<t)-1,[t]),j=m.useCallback(()=>{te(b(r,o.current,i.current))},[r]),D=m.useMemo(()=>ye(t,n,r),[t,n,r]),A=m.useCallback(()=>{const u=R.current;let f=K(t,S.current);new Array(Math.floor(Math.random()*4)).forEach(()=>f=_(f));const d=z(t,u);R.current=f,p.current=u,i.current=c(!0),i.current.unshift(...d),i.current.splice(n)},[c,n,t]),I=u=>{const f={left:1,right:-1,bottom:0}[u]||0,d=i.current,g=D(u,o.current,i.current);d!==g&&(i.current=g,p.current.rightSpace+=f,j())},B=()=>{const u=p.current;p.current=_(p.current);const f=z(t,p.current),d=o.current;if(f.some((g,C)=>{const N=d[C-r];return N&&N&g})){p.current=u;return}i.current=[...f].concat(new Array(n-f.length).fill(0)),j()};V("keydown",u=>{if(u.code==="ArrowLeft")I("left");else if(u.code==="ArrowRight")I("right");else if(u.code==="ArrowUp")B();else if(u.code==="ArrowDown"){if(l.current)return;l.current=!0,s.current/=4}}),V("keyup",u=>{u.code==="ArrowDown"&&(l.current=!1,s.current*=4)});const P=m.useCallback(()=>{y().setGameStatus(h.OVER);const u={gameId:y().gameId,score:S.current};se(J,{...u,time:Date.now()}),ce(),v.event("game-elsfk-over",u),v.expose("game-elsfk-over"),ge({id:"reload-history",type:xe.RELOAD_HISTORY}),ae().showMessage({content:`游戏结束, 得分: ${S.current}`})},[]),k=m.useMemo(()=>ie(()=>{y().gameStatus===h.RUNNING&&setTimeout(()=>{try{i.current=D("bottom",o.current,i.current),p.current.topPos+=1}catch(f){const d=f;if(!d.custom)return v.error("elsfk-next-tick",f);if(d.action!=="submit")return;const g=b(r,o.current,i.current);o.current=g;const C=Ae(e,g,H);if(C.length){S.current+=C.length*t,G(C),setTimeout(()=>{C.forEach(N=>{g.splice(N,1),g.unshift(0)}),G([]),A(),k()},2e3);return}if(i.current[n-e-1])return P();A()}j(),k()},s.current)},10),[D,A,H,r,n,e,t,j,P]),L=m.useCallback((u=!1)=>{o.current=c(!1),i.current=c(!0),s.current=500,l.current=!1,S.current=0,R.current=u?{}:K(t,S.current)},[c,t]),ne=m.useCallback(()=>{y().gameStatus===h.OVER&&(y().setGameStatus(h.RUNNING),L(),A(),k())},[A,k,L]);return Ue(({type:u})=>{const{setGameStatus:f,gameStatus:d}=y();u===E.START?d===h.PAUSE?(f(h.RUNNING),k()):ne():u===E.PAUSE?f(h.PAUSE):u===E.RELOAD&&(L(!0),f(h.OVER),j())}),a.jsx(ue,{onFirstAppear:()=>v.expose("game-elsfk"),children:a.jsxs(w,{$flex:"1",$alignItems:O.CENTER,$justifyContent:$.CENTER,$gap:"1",$direction:M.COLUMN,children:[a.jsxs(De,{children:["当前得分: ",S.current]}),a.jsxs(Fe,{$flex:"1",$gap:"1",$justifyContent:$.CENTER,style:{width:"100%"},children:[a.jsx(w,{$justifyContent:$.CENTER,children:a.jsx(Z,{$gap:"0.5",$direction:M.COLUMN,children:U.map((u,f)=>a.jsx(Q,{rowData:u,$isClear:re.includes(f)},f))})}),a.jsxs(Oe,{$direction:M.ROW,$justifyContent:$.BETWEEN,children:[a.jsx(Te,{move:I,rotate:B}),a.jsx(Le,{elementInfo:R.current})]})]})]})})}const ft={title:"俄罗斯方块",needBackIcon:!0,rightArea:a.jsx(Ge,{})};export{mt as Component,ft as handle};
